...
_JALALI_RE = re.compile(r"^\s*(\d{3,4})[/\-](\d{1,2})[/\-](\d{1,2})\s*$")

def normalize_jalali_date(value, default=None):
    """
    ورودی آزاد کاربر (مثلاً 1404/5/9 یا 1404-05-09) را به فرمت یکنواخت
    YYYY/MM/DD (با صفر ابتدایی) تبدیل می‌کند تا مقایسه، مرتب‌سازی و
    جستجوی متنی تاریخ‌ها در دیتابیس همیشه درست کار کند.
    اگر ورودی نامعتبر باشد، default برگردانده می‌شود.
    """
    if not value:
        return default
    m = _JALALI_RE.match(str(value))
    if not m:
        return default
    y, mo, d = m.groups()
    try:
        y, mo, d = int(y), int(mo), int(d)
        if not (1 <= mo <= 12) or not (1 <= d <= 31) or y < 1300 or y > 1500:
            return default
        # اعتبارسنجی واقعی تاریخ با تقویم جلالی (تاریخ‌های ناموجود مثل 31 آبان را رد می‌کند)
